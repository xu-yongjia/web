    ...
    <div class="content">
      ...
      <div class="section-address">
        ...
        <div class="address-body">
          <ul>
            <li
              :class="item.index == confirmAddress ? 'in-section' : ''"
              v-for="item in address"
              :key="item.id"
              @click="confirmAddress=item.index"
            >
              <h2>{{item.name}}</h2>
        ...
        <div class="goods-list">
          <ul>
            <li v-for="item in getCheckGoods" :key="item.id">
              ...
              <span class="pro-price">{{item.discount_price}}元 x {{item.number}}</span>
              <span class="pro-status"></span>
              <span class="pro-total">{{item.discount_price * item.number}}元</span>
            </li>
          </ul>
        </div>
      </div>
        ...
        <div class="money-box">
          <ul>
            ...
            <li class="total">
              ...
              <span class="value">
                <span class="total-discount_price">{{getTotalPrice}}</span>元
              </span>
            </li>
          </ul>
        </div>
...
export default {
    ...
    this.$axios
        ...
        .then(res => {
          if (res.status === 200) {
            ...
            //token过期，需要重新登录
            this.loginExpired(res.data.msg)
          } else {
            this.notifyError('获取收货地址失败', res.data.msg)
          }
        })
        .catch(err => {
  ...
  methods: {
    ...
    addOrder() {
    let cart = [];
// 使用 Array.map() 方法遍历 this.getCheckGoods 数组
    this.getCheckGoods.map(item => {
  // 提取每个元素的 cart_id 属性并添加到 cartIds 数组中
    ...
    console.log("createOrder")
      this.$axios
        .post("/api/users/createOrder", {
          carts: cart,
          username:this.address[parseInt(this.confirmAddress)-1].name,
          user_id:this.address[parseInt(this.confirmAddress)-1].user_id,
          phone:this.address[parseInt(this.confirmAddress)-1].phone
        })
        .then(res => {
          ...
          switch (res.data.status) {
            ...
            case 200:
              for (let i = 0; i < products.length; i++) {
                ...
                // 删除已经结算的购物车商品
                this.deleteShoppingCart(temp.cart_id);
              }
              // 提示结算结果
