  ...
  <div class="shoppingCart">
    ...
    <div class="content" v-if="getShoppingCart.length>0">
      <ul>
        ...
        <!-- 购物车列表 -->
        <li class="product-list" v-for="(item,index) in getShoppingCart" :key="item.cart_id">
          <div class="pro-check">
            <el-checkbox :value="item.check" @change="checkChange($event,index)"></el-checkbox>
          </div>
          <div class="pro-img">
            <router-link :to="{ path: '/goods/details', query: {product_id:item.product_id} }">
              <img :src="item.img_path" />
            </router-link>
          </div>
          <div class="pro-name">
            <router-link
              :to="{ path: '/goods/details', query: {product_id:item.product_id} }"
          ...
          <div class="pro-total pro-total-in">{{item.discount_price*item.number}}元</div>
...
export default {
  ...
  methods: {
    ...
    // 修改商品数量的时候调用该函数
    handleChange(currentValue, key, product_id) {
      // 当修改数量时，默认勾选
